      ...
      integer*4 ierr
c
      integer*4 i,j,k,id,is,isg,ir,ig,nd,nt0,nf0,ntcut0,nfcut0,ishift
      integer*4 lf,lf1,istp,ldeg,ldegf,ldeg0,lfused
      integer*4 istat,ldegup,ldeglw,ldegneed
      real*8 sarc,anorm,slwcut,sigma,delta
...
c
      do is=1,ns
        isg=1
        sigma=dabs(grndep(1)-deps(is))
        do ig=2,ngrn
          if(dabs(grndep(ig)-deps(is)).lt.sigma)then
            isg=ig
            sigma=dabs(grndep(ig)-deps(is))
          endif
        enddo
c
        sarc=(rr0+sigma)/rrr
...
c
          if(dis(is,ir).le.sarc)then
            idr(is,ir)=0
          else if(dis(is,ir).ge.PI.or.sarc.le.0.d0)then
            idr(is,ir)=ndmax
          else
            delta=dlog(PI/sarc)/dble(ndmax)
            idr(is,ir)=min0(ndmax,1+idint(dlog(dis(is,ir)/sarc)/delta))
          endif
c
c          idr(is,ir)=0
c
...
c
      do is=1,ns
c        call swavelet(trss(is),df,fi,nf,wvf(1,is))
        call brunewvlet(trss(is),df,fi,nf,2,wvf(1,is))
      enddo
c
      do lf=1,nf
        do ir=1,nr
...
c
      do ig=1,ngrn
        ...
        write(*,'(a)')' '
        write(*,'(a,i4,a,f9.3,a)')' processing ',1+isg2(ig)-isg1(ig),
...
c
c          ldegf=1000+idint(dble(ldegup-1000)*f/fcut)
c
          ldegneed=0
          ldeglw=ldegmax
          do is=isg1(ig),isg2(ig)
