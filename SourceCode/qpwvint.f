...
c
      ...
      complex*16 cfac,ca,cb,dur,dut,dup,eii,dpt,dgr,dgt,dgp
...
c
c             add output of gravitational potential (R. Wang, 14. Oct 2020)
c
              ptlm(ldeg,istp,0)=pl0(ldeg,istp)                          !m=0,  1
...
c             add output of gravitational potential (R. Wang, 14. Oct 2020)
c
              ptlm(ldeg,istp,0)=pl0(ldeg,istp)                          !m=1,  1
...
c           add output of gravitational potential (R. Wang, 14 Oct 2020)
c
            ptlm(ldeg,6,0)=pl0(ldeg,6)                                  !m=2,  1
...
c
     ...
     &                       -urlm(1,istp,id-1)/c3
              ptlm(0,istp,id)=ptlm(0,istp,id-1)
     &                       -ptlm(1,istp,id-1)/c3
...
c
     ...
     &                         -cb*urlm(ldeg-1,istp,id-1)
                ptlm(ldeg,istp,id)=ptlm(ldeg,istp,id-1)
     &                         -ca*ptlm(ldeg+1,istp,id-1)
     &                         -cb*ptlm(ldeg-1,istp,id-1)
...
c
     ...
     &                    -cb*uplm(ldeg-1,6,id-1)
              ptlm(ldeg,6,id)=ptlm(ldeg,6,id-1)
     &                    -ca*ptlm(ldeg+1,6,id-1)
     &                    -cb*ptlm(ldeg-1,6,id-1)
...
c
                dpt=sf3(is)*ptlm(ldeg,1,id)*cp0
     &             +expl(is)*ptlm(ldeg,2,id)*cp0
     &             +clvd(is)*ptlm(ldeg,3,id)*cp0
     &             +(sf1(is)*csa(is,ir)+sf2(is)*ssa(is,ir))
     &              *ptlm(ldeg,4,id)*cp1*ssd(is,ir)
     &             +(ds31(is)*csa(is,ir)+ds23(is)*ssa(is,ir))
     &              *ptlm(ldeg,5,id)*cp1*ssd(is,ir)
     &             +(ss12(is)*ss2a(is,ir)+ss11(is)*cs2a(is,ir))
     &              *ptlm(ldeg,6,id)*cp2*ssd(is,ir)**2
c
