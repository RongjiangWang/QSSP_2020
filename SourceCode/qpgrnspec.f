...
c
      ...
      real*8 bet,k2r,n2r,grr,vp2,alf,cxi
...
c
      write(*,*)' '
      write(*,'(a,i3,a,f9.3,a)')' ... calculate Green functions for ',
...
c
          ...
          enddo
          if(omi.gt.0.d0)then
            do ly=min0(lys,lyr,lyos)-1,1,-1
              bet=dlog(roup(ly)/rolw(ly))/(rrup(ly)-rrlw(ly))
              k2r=dble(ldeg)*dble(ldeg+1)/rrup(ly)**2
              grr=dreal(cgrup(ly))
              vp2=vpup(ly)**2
              if(isotherm)then
                n2r=0.d0
              else
                n2r=-grr*(bet+grr/vp2)*dsin(0.5d0*PI*f/fbvatm)**3
              endif
              alf=0.5d0*(grr/vp2-1.d0/rrup(ly)+n2r/grr)
              cxi=0.25d0*(grr/vp2-1.d0/rrup(ly)-n2r/grr)**2
     &           +(k2r-omi**2/vp2)*(1.d0-n2r/omi**2)
              if(cxi.gt.0.d0)then
                expo=expo+(dsqrt(cxi)-alf)*(rrup(ly)-rrlw(ly))
                if(expo.gt.expos)then
                  lyupp(ldeg)=ly
                  goto 201
                endif
...
c
          lylwp(ldeg)=ly0
          expo=0.d0
          do ly=max0(lys,lyr,lyob)+1,ly0-1
            if(vsup(ly).gt.0.d0)then
              ksp=omi/vsup(ly)
            else
              ksp=omi/vpup(ly)
